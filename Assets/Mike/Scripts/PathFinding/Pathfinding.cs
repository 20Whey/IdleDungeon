...
public class Pathfinding {
	...
	{
		Instance = this;
		grid = new GridClass<PathNode>(width, height, 0.5f, new Vector2(-9, -5), (GridClass<PathNode> g, int x, int y) => new PathNode(g, x, y));
	}
	...
	public List<Vector2> FindPath(Vector2 startWorldPosition, Vector2 endWorldPosition)
	{
		...
		if (path == null) {
			...
			{
				vectorPath.Add(new Vector2(pathNode.x + 0.5f, pathNode.y + 0.5f));
			}
			return vectorPath;
		}
	}
